 ...
 * extension.
 */
class CRM_Oapproviderlistapp_ExtensionUtil {
  const SHORT_NAME = "oapproviderlistapp";
  const LONG_NAME = "biz.jmaconsulting.oapproviderlistapp";
  const CLASS_PREFIX = "CRM_Oapproviderlistapp";
...
/**
 * @return CRM_Oapproviderlistapp_Upgrader
 */
function _oapproviderlistapp_civix_upgrader() {
  if (!file_exists(__DIR__ . '/CRM/Oapproviderlistapp/Upgrader.php')) {
    return NULL;
  }
  else {
    return CRM_Oapproviderlistapp_Upgrader_Base::instance();
  }
}
...
function _oapproviderlistapp_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_oapproviderlistapp_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
...
function _oapproviderlistapp_civix_civicrm_angularModules(&$angularModules) {
  ...
  $files = _oapproviderlistapp_civix_glob(__DIR__ . '/ang/*.ang.php');
 ...
 * missing them.
 */
function _oapproviderlistapp_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _oapproviderlistapp_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _oapproviderlistapp_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _oapproviderlistapp_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
